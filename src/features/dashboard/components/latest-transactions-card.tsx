...
export function LatestTransactionsCard() {
  ...
  const [items, setItems] = React.useState<LatestTransactionItem[]>([])
            ...
            <div className="border-t">
              {visibleItems.map((item) => {
                ...
                return (
                  <div
                    key={item.id}
                    // Mobile: 3 cols (icon | title+category | amount+date)
                    // md+:  4 cols (icon | title+category | groupName | amount+date)
